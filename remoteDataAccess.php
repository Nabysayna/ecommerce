<?php 
	...
	class RemoteDataAccess{
		...
		private $_servername = "db708233152.db.1and1.com";
		private $_connexion ;

		public function __construct(){

			try {
				    $this->_connexion = new PDO("mysql:host=$this->_servername;dbname=db708233152",'dbo708233152','E-c0mm3rc3');
		...
		public function getNouveute(){
			$req="SELECT * FROM pageprincipale WHERE categorie LIKE '%nouveaute%'";
		...
		public function getEnvedette(){
			$req="SELECT * FROM pageprincipale WHERE categorie LIKE '%en vedette%'";
			$resultat= $this->_connexion->query($req);
			return $resultat;
			}
		public function getHabitFemme(){
			$req="SELECT * FROM pageprincipale WHERE categorie LIKE '%femme%'";
			$resultat= $this->_connexion->query($req);
			return $resultat;
			}
		public function getHabitHomme(){
			$req="SELECT * FROM pageprincipale WHERE categorie LIKE '%homme%'";
			$resultat= $this->_connexion->query($req);
			return $resultat;
			}
		public function getSac(){
			$req="SELECT * FROM pageprincipale WHERE categorie LIKE '%sac%'";
			$resultat= $this->_connexion->query($req);
			return $resultat;
			}
		...
		public function getelectronique(){
			$req="SELECT * FROM pageprincipale WHERE categorie LIKE '%electronique%'";
			$resultat= $this->_connexion->query($req);
			return $resultat;
			}
